...
- **OAuth redirect schemes** — custom app schemes are an exact allowlist. For local/dev named desktop bundles,
  add comma-separated schemes via `OMI_AUTH_ALLOWED_CUSTOM_REDIRECT_SCHEMES`; never use wildcard scheme acceptance.
